...
From annotations and assembly files -> extracted upstream sequences of given length minimum length 15nt cutoff at 250nt. From families with specific conditions. X families identified, In each family CDSs were retrieved, aligned using protein-guided alignment (TranslatorX) and ML tree (PhyML) was computed -> used BigFoot to consider the phylogenetic signal + Comparison with MEME.

From this, X motifs matched partially (quantify!) between MEME and BigFoot -> search through the genome this motifs in each species.
Using publicly available RNA expression data, search correlations between motifs and expression level.
\begin{definition}
